...
function prepareVol1Data(current, parent, datas, links) {
  ...
  if (parent) {
    links.push({
      source: 'dvol1-' + parent.id,
      target: 'dvol1-' + current.id,
      symbolSize: [10, 10],
      label: {
        formatter: '包含',
        show: true,
...
// 选中 vol1 条目后，显示其祖先条目 + 儿子条目
function selectVol1Series(id, chart) {
  chart.showLoading();
  wx.request({
    url: `https://www.skyatlas.net/webAppICD/api/d1/getSeries?id=${id}`,
    header: {
...
function initChart(canvas, width, height, dpr) {
  ...
  chart.showLoading();
  wx.request({
    url: 'https://www.skyatlas.net/webAppICD/api//d1/chapters',
    header: {
      Accept: 'text/json;charset=utf-8'
    },
    method: 'GET',
    success: function (res) {
      ...
      chart.on('click', function (params) {
        ...
        if (params.componentType == 'series' &&
          ...
          selectVol1Series(params.data.id.replace('dvol1-', ''), chart);
        }
      });
      
      chart.on('click','series-graph.label',function(params){
        console.log('series.graph.label click ', params);
      })
    },
    fail: function (err) {
